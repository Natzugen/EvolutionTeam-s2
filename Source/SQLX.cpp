...
{
	szDriver="{SQL Server}";
	GetPrivateProfileStringA("SQLServer","Host","127.0.0.1",szServer,sizeof(szServer),"..\\EData\\ESQL.ini");
	GetPrivateProfileStringA("SQLServer","Host","127.0.0.1",szServer2,sizeof(szServer2),"..\\EData\\ESQL.ini");
	GetPrivateProfileStringA("SQLServer","User","sa",szUser,sizeof(szUser),"..\\EData\\ESQL.ini");
	GetPrivateProfileStringA("SQLServer","Password","password",szPassword,sizeof(szPassword),"..\\EData\\ESQL.ini");
	GetPrivateProfileStringA("SQLServer","Database","MuOnline",szDatabase,sizeof(szDatabase),"..\\EData\\ESQL.ini");

	if(!this->Connect())
	{
		MessageBoxA(0,"Failed to connect - Check your ESQL.ini!","Error",MB_OK);
		::ExitProcess(0);
	} 
}
